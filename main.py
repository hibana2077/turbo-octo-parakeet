...
def setup(args, prefix_saved_mode):
    ...
    logger.info("{}".format(model.backbone_cfg))
...
def train(args, model,cp_mask, prefix_saved_mode):
    ...
    ad_net_local = AdversarialNetwork(config.head_dim, config.head_dim)
    ...
    for global_step in range(1, t_total):
        ...
        if( not args.use_cp ):
            cp_mask = torch.ones_like(cp_mask, device=args.device)
...
def main():
    ...
    parser.add_argument("--use_jfpd", default=False, action="store_true",
                        help="Use JFPD loss on target features with source prototypes.")
    ...
    parser.add_argument("--jfpd_alpha", default=0.5, type=float,
                        help="Alpha for JFPD combination of feature/prediction divergence.")
    ...
    num_patchs = (args.img_size // model.patch_size) ** 2
    cp_size = num_patchs + model.prefix_tokens
